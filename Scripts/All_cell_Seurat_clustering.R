## Clustering analysis for E7.5-E15.5 cells with more than 6,000 expressed genes
...
# Get subset of data
cell_anno <- cell_anno[cell_anno$num_genes_expressed>6000,]
...
PCHeatmap(mus, pc.use = 1:15, cells.use = 200, do.balanced = TRUE, label.columns = FALSE, use.full = FALSE)
PCElbowPlot(mus)

# Cluster the cells
mus <- FindClusters(mus, reduction.type = "pca",dims.use = 1:14, save.SNN = T,resolution = 1.2,temp.file.location="/hwfssz1/ST_PRECISION/USER/zhongyu/1.Project/Mus_Liver/seurat/results/")
PrintFindClustersParams(object = mus)

# Run non-linear dimensional reduction (tSNE)
mus <- RunTSNE(mus, dims.use = 1:14, do.fast = T)
TSNEPlot(mus,pt.size = 1.5,colors.use = c("#E41A1C","#377EB8","#4DAF4A","#984EA3","#FF7F00","#FFFF33","#A65628","#999999","#F781BF","#191970","#90EE90","#E7298A"))
...
DoHeatmap(mus, genes.use = top20$gene, slim.col.label = TRUE, remove.key = FALSE,cex.row=5)

# Lable cells with embryo stages
cluster <- factor(cell_anno$Stage,levels=c("E7.5","E8.5","E9.5","E10.0","E10.5","E11.5","E12.5","E13.5","E14.5","E14.5_GFPn","E15.5"))
names(cluster)<-rownames(cell_anno)
mus@ident <- cluster
TSNEPlot(mus,pt.size = 1.5,colors.use =  c("#E41A1C","#377EB8","#4DAF4A","#984EA3","#FF7F00","#FFFF33","#A65628","#F781BF","#191970","#90EE90","#E7298A"))

save.image("mus_all_sc_seurat_analysis.RData")
