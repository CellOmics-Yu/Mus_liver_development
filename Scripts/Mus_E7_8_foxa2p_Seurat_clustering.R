...
# Get Foxa2+ cells from E7.5-E8.5 embryo
cell_anno <- cell_anno[(cell_anno$Stage=="E7.5"|cell_anno$Stage=="E8.5")&cell_anno$num_genes_expressed>6000&cell_anno$Foxa2_EGFP=="Express",]
...
PCAPlot(object = mus, dim.1 = 1, dim.2 = 2,pt.size = 10)
PCHeatmap(mus, pc.use = 1:20, cells.use = 40, do.balanced = TRUE, label.columns = FALSE, use.full = FALSE)
PCElbowPlot(mus)

# Cluster the cells
mus <- FindClusters(mus, reduction.type = "pca",dims.use = 1:12, save.SNN = T,resolution = 1,temp.file.location="/hwfssz1/ST_PRECISION/USER/zhongyu/1.Project/Mus_Liver/seurat/results/",k.param=5,k.scale=5)
PrintFindClustersParams(object = mus)

# Run non-linear dimensional reduction (tSNE)
mus <- RunTSNE(mus, dims.use = 1:12, do.fast = T,perplexity = 10)
TSNEPlot(mus,pt.size = 1.5,colors.use = c(colors(8,"Set1")[c(1:5,7,8)],colors(8,"Dark2")))
...
cluster <- factor(cell_anno$Stage,levels=c("E7.5","E8.5"))
names(cluster)<-rownames(cell_anno)
mus@ident <- cluster
TSNEPlot(mus,pt.size = 1.5,colors.use =  c("#E41A1C","#377EB8"))

save.image("E7_8_foxa2p_seurat_analysis.RData")
